...
<body class="article_bg">
	...
	<div>
		<form method="post" target="uploadResultFrame" enctype="multipart/form-data" action="<%=FILE_URL%>/file/FileUploadManage.do?appCode=PDM&infoType1=<%=InfoTypeConfig.INFO_TYPE_PRODUCT_ITEM %>&infoId1=<%=bean.getItemId()%>&fileId=<%=productImageBean != null ? productImageBean.getFileId() : 0%>">
	...
	<form id="formData" name="formData" method="post" action="/pdm/ProductItemImageManage.do?" onsubmit="javascript:return checkForm('formData');" >
		<input id="infoId" name="infoId" value="<%=productImageBean != null ? productImageBean.getInfoId() : 0%>" type="hidden" />
		<input id="itemId" name="itemId" value="<%=bean.getItemId()%>" type="hidden" />
